...
void draw() {
  ...
  layer.beginDraw();
//  //layer.background(-1,0); // Kids didn't like Grid inthe background so I have disabled it instead of this I'll textures 
    ...
    for (int j=0;j<keys.size();j++) {
      ...
      for (int x=0;x<handopen.width;x++) {
        for (int y=0;y<handopen.height;y++) {
          ...
          else handopen.pixels[i]=color(0);
        }
      }
      for (int x=0;x<handclose.width;x++) {
        for (int y=0;y<handclose.height;y++) {
          ...
          else handclose.pixels[i]=color(0);
        }
      }
...
void mouseReleased() {
  // d.stopDragging();
  c = new Client(this, "10.14.27.11", 5001);
  start=true;
}
