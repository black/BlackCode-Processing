...
void setup()
{
  ...
  c = new Client(this, "10.113.149.43", 5001); // create a client instant if you are already started make sure you press a mouse click so it'll start to create a new client instant change client ip address.
...
void draw() {
  ...
  {    
    ...
    for (int j=0;j<keys.size();j++) {//-************************************
      ...
      // --------------- enable root brush ------------------------------
      if (openPalm.get(handId)!=null && openPalm.get(handId)  &&  dist(int(linex-20), int(liney-20), width/2-BrushB.width/2-200, height/2-BrushB.height-10) < BrushB.width ) { // check if Icon gets selected or not
        rootmode =!rootmode;
        drawmode =false;
        networkmode = false;
        buttonplayer.play();
      ...
      // --------------- enable Eraser  ---------------------------
      if (openPalm.get(handId)!=null && openPalm.get(handId)  &&  dist(int(linex-20), int(liney-20), width/2-BrushB.width/2, height/2) < BrushB.width ) { // check if Icon gets selected or not
        erasemode =!erasemode;
        drawmode =false;
        buttonplayer.play();
...
void mouseReleased() {
  // d.stopDragging();
  c = new Client(this, "10.113.149.43", 5001);
  start=true;
}
