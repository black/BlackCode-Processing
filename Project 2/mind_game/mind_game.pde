...
void draw() {
  ...
  for (int i = Part.size()-1;i >= 0; i--) {
    for (int j= Part.size()-1;j >= 0; j--) {
      ...
      Particle a=(Particle)Part.get(i);
      Particle b=(Particle)Part.get(j);
...
void mousePressed() {
  Part.add(new Particle(mouseX, mouseY ));
}
 
void keyPressed() {
  if (key == 'x') {
    for (int i=0;i<10;i++) {
      Part.add(new Particle(random(width), random(height) ));
    }
  }
  if (key == 'z' && (Part.size() >20) ) {
    for (int i=0;i<9;i++) {
      Part.remove(i);
    }
  }
