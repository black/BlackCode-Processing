...
{ background(0); 
  ...
  { float w = width*1.0/numSamples;
    ...
    { int v = ((Integer)attSamples.get(i)).intValue();
     ...
     stroke(255); line(0, height/2, width, height/2); 
}  
    fill(255, 0, 0, 100); 
    stroke(255, 100); 
    rect(width/2-50, height-attention*height/100.0, 100, attention*height/100.0); 
    //signal strength stroke(255); 
    line(0, 50, 50, 50); fill(0, 255,0); 
...
{ 
  strength = sig; println(sig); 
 } 
 
 public void attentionEvent(int attentionLevel) 
{ //println("Attention Level: " + attentionLevel); 
  attention = attentionLevel; attSamples.add(new Integer(attention));
