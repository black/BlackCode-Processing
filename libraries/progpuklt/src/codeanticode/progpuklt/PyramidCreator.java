...
class PyramidCreator
 {
    ...
    void allocate(int w, int h, int n, boolean lod)
    {
        width = w;
        height = h;
        nLevels = n;
        ...
        for (int level = 0; level < nLevels; ++level)
        {
            int W = (w >> level);
            int H = (h >> level);
            gl.glTexImage2D(textureTarget, level, GL.GL_RGBA8, W, H, 0, GL.GL_RGBA, GL.GL_UNSIGNED_BYTE, null);
        }

        Util.checkGLErrors("PyramidCreator.allocate()");

        gl.glBindTexture(textureTarget, tmpTexID[0]);
        gl.glTexParameteri(textureTarget, GL.GL_TEXTURE_MAG_FILTER, GL.GL_NEAREST);
        gl.glTexParameteri(textureTarget, GL.GL_TEXTURE_MIN_FILTER, GL.GL_NEAREST);
        //gl.glTexParameteri(textureTarget, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);
        gl.glTexParameteri(textureTarget, GL.GL_TEXTURE_WRAP_S, GL.GL_CLAMP_TO_EDGE);
        gl.glTexParameteri(textureTarget, GL.GL_TEXTURE_WRAP_T, GL.GL_CLAMP_TO_EDGE); 
        ...
        gl.glTexParameteri(textureTarget, GL.GL_TEXTURE_WRAP_S, GL.GL_CLAMP_TO_EDGE);
        gl.glTexParameteri(textureTarget, GL.GL_TEXTURE_WRAP_T, GL.GL_CLAMP_TO_EDGE); 
        gl.glTexImage2D(textureTarget, 0, GL.GL_RGBA8, w, h, 0, GL.GL_RGBA, GL.GL_UNSIGNED_BYTE, null);
...
//    	int textureTarget = GL.GL_TEXTURE_2D;
//    	gl.glBindFramebufferEXT(GL.GL_FRAMEBUFFER_EXT, fbID[0]);
//    	gl.glFramebufferTexture2DEXT(GL.GL_FRAMEBUFFER_EXT, GL.GL_COLOR_ATTACHMENT0_EXT, textureTarget, pyrTexID[0], level);
    ...
    void buildPyramidForGrayscaleImageInit(int textureTarget)
    {
        ...
        {
           pass1HorizShader = new CgFragmentProgram("PyramidCreator.buildPyramidForGrayscaleImage.pass1HorizShader");
           pass1HorizShader.setProgramFromFile("pyramid_pass1v.cg");
           pass1HorizShader.compile();
           Util.checkGLErrors("creating pyramid_pass1v.cg");
        }

        if (pass1VertShader == null)
        {
           pass1VertShader = new CgFragmentProgram("PyramidCreator.buildPyramidForGrayscaleImage.pass1VertShader");
           pass1VertShader.setProgramFromFile("pyramid_pass1h.cg");
           pass1VertShader.compile();
           Util.checkGLErrors("creating pyramid_pass1h.cg");
        }

        if (pass2Shader == null)
        {
           pass2Shader = new CgFragmentProgram("PyramidCreator.buildPyramidForGrayscaleImage.pass2Shader");
           pass2Shader.setProgramFromFile("pyramid_pass2.cg");
           int val = explicitLod ? 1 : 0;
           String[] args = { "-DEXPLICIT_LOD=" + val, null };
           pass2Shader.compile(args);
           Util.checkGLErrors("creating pyramid_pass2.cg");
        }
	...
	void buildPyramidForGrayscaleImageExec(int textureTarget)
	{
        ...
        Util.checkGLErrors("starting pass2Shader");
        ...
        for (int level = 1; level < nLevels; ++level)
        {
            ...
            //gl.glFramebufferTexture2DEXT(GL.GL_FRAMEBUFFER_EXT, GL.GL_COLOR_ATTACHMENT0_EXT, textureTarget, tmpTexID[0], level-1);
    ...
    protected int width, height, nLevels;
    protected boolean explicitLod;
