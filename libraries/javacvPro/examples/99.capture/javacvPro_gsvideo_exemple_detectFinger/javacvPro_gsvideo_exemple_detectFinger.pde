...
// Code type capture GSVideo - utilisation possible aussi de captureEvent() 
  ...
  if (cam.available() == true) { // si une nouvelle frame est disponible sur la webcam
    ...
    image(opencv.getMemory2(),widthCapture,heightCapture); // affiche image résultante stockée dans le mémory 2
       ...
       int nombreDoigts=opencv.detectFinger(cdArray, false); 
       // la valeur renvoyée est au format pouce + nombre doigts soit 12 pour le pouce + 2 doigts.
       
       println("Valeur renvoyée par la fonction de détection des doigts : " + nombreDoigts + "."); 

       if (nombreDoigts>=10) {
               println("Le pouce et  " + (nombreDoigts-10) + "doigts ouverts."); 
       }
       else if (nombreDoigts<5) {
               println("Le pouce fermé et  " + (nombreDoigts) + "doigts ouverts."); 
         
       }
       else if (nombreDoigts==5) {
               println("Les "+ (nombreDoigts) + "doigts ouverts."); 
