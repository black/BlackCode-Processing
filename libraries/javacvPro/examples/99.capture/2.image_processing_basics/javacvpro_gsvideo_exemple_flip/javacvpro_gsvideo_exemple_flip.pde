...
void setup(){ // fonction d'initialisation exécutée 1 fois au démarrage
        ...
        size (widthCapture, heightCapture*2); // crée une fenêtre Processing de la 2xtaille du buffer principal OpenCV
...
// Code capture GSVideo 
  ...
  if (cam.available() == true) { // si une nouvelle frame est disponible sur la webcam
        ...
        opencv.flip(opencv.VERTICAL); // applique renversement vertical sur le buffer principal OpenCV
        //opencv.flip(opencv.HORIZONTAL); // applique renversement horizontal sur le buffer principal OpenCV
        //opencv.flip(opencv.BOTH); // applique retournement renversement vertical+horizontal sur le buffer principal OpenCV
        
        println("Durée traitement image par OpenCV=" + (millis()-millis0)+" ms."); 
        
        //--- affiche image finale --- 

        image(opencv.getBuffer(),0,heightCapture); // affiche le buffer principal OpenCV dans la fenêtre Processing        
