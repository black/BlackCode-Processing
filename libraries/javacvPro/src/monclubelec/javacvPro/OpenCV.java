...
public class OpenCV { // crée la classe
	...
	public static opencv_objdetect.CvHaarClassifierCascade cascade; 
	...
	//---- objets utiles video 
	static FrameGrabber grabber=null; // objet pour la capture video 
	...
	private void welcome() {
		System.out.println("javacvPro (Processing library) - version 0.5 - by X.HINAULT - Octobre 2012 - www.mon-club-elec.fr - (c) all rights reserved - GPLv3 ");
	...
	// V2 -- 09/2011 -- passe par un ByteBuffer --- plus rapide -- 14ms ! -- + gestion du nombre canaux... 
	/*	
		  PImage imgOut; // crée objet PImage
	 ...
	 // --- fin V2  
	   ...
	   */
		...
		//------ v3 --- mars 2012 --- passe par un bufferedImage - très simple.. 8ms
		
		
		BufferedImage bimg= IplImageIn.getBufferedImage();
		
		PImage imgOut=new PImage(bimg.getWidth(),bimg.getHeight(),PConstants.ARGB);
	    bimg.getRGB(0, 0, imgOut.width, imgOut.height, imgOut.pixels, 0, imgOut.width);
	    imgOut.updatePixels();
		
	    // d'après : http://forum.processing.org/topic/converting-bufferedimage-to-pimage
...
*/	
	...
	public void loadImage (String cheminFichierIn) {
		...
		//----- en utilisant fonction loadImage de Processing 
		PImage imgLoad = p.loadImage(cheminFichierIn); // charge le fichier via Processing		
		allocate(imgLoad.width, imgLoad.height,true,true); // initialise les buffers et le Memory... 
		
		copy(imgLoad); // charge l'image dans le Buffer OpenCV 
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public Blob[] selectBlobs(Blob[] blobsIn, boolean hwTestIn, float ratioWHTest, float delta, boolean areaTestIn, float areaRatioTest, boolean modeIn, boolean debugIn) {
		...
		// delta : pourcentage de variation en % de la valeur W/H cible
		...
		for (int i=0; i<blobsIn.length; i++ ) { // passe en revue les Blob du tableau reçu par la fonction
	        ...
	        float ratioWH=(float)rectangleBlob.width/(float)rectangleBlob.height; // calcule le ratio largeur/hauteur
	        //float ratioHW=(float)rectangleBlob.height/(float)rectangleBlob.width; // calcule le ratio hauteur/largeur
	        // NB ++ : les ratios doivent être des Float !!

	        
	        float aireBlob=blobsIn[i].area; // récupère l'aire de la forme courante

        	if (debugIn) PApplet.println ("aire Blob="+aireBlob);

	        
	        float ratioAire=(float)aireBlob/((float)rectangleBlob.width*(float)rectangleBlob.height); // calcul du ratio Aire forme / Aire rectangle
	        // aire cercle = pi * r²
	        // aire carré = (2r)²=4r²
	        ...
	        if ((hwTestIn) && (areaTestIn)) { // si test HW et test area activés simmultanément 
	        	...
	        	if (debugIn) PApplet.println ("ratioAire="+ratioAire);

	        	//--- compliqué non --- 
	        	//if ( ( (ratioWH>ratioWHTest) || (ratioHW>ratioHWTest) )  && (ratioAire>areaRatioTest)  ) { 
	        	/*if ( 
	        		((ratioWH<(1+(1-ratioWHTest)))
	        				&&(ratioWH>ratioWHTest)
	        				&&(ratioAire>areaRatioTest)) // 1 +/- x%
		        ...
		        */
	        	...
	        	/*-- plus simple -- ? 
	        	float ratio=0; // variable ratio pris en compte
	        	if (ratioWH<ratioHW) ratio=ratioWH; else ratio=ratioHW; // on garde le ratio le plus petit
	        	
	        	if (debugIn) PApplet.println ("ratio="+ratio);
	        	if ((ratio>ratioHWTest)&&(ratioAire>areaRatioTest)) { // si le ratio est sup au ratio réf et aire aussi
	        		
	        		//ajoute un Blob à ArrayList de Blob
		        	selectBlobsList.add(blobsIn[i]); // ajoute le blb à l'ArrayList 

	        	} // fin if ratio et area
				*/
	        	
	        	//--- v3 qui utilise cible W/H et delta - + lisible 
	        	if ( 
		        		(ratioWH<(ratioWHTest+(ratioWHTest*delta/100)))
		        		 && (ratioWH>(ratioWHTest-(ratioWHTest*delta/100))) 
		        				&&(ratioAire>areaRatioTest) // 1 +/- x%
	        ...
	        if ((hwTestIn) && (!areaTestIn)) { // si test HW actif et test Aire pas actif
	        	...
	        	if ((ratioWH<(ratioWHTest+(ratioWHTest*delta/100)))
		        		 && (ratioWH>(ratioWHTest-(ratioWHTest*delta/100))) 
	        			){
	        		// si ratio W/H +/- delta conforme 
	...
	public Blob[] selectBallBlobs(Blob[]blobsIn, boolean debugIn) {
		
		//les Blobs ayant caractéristiques proche du carré et forme proche du cercle = sélection ball
		Blob[] myBlobsOut=selectBlobs(blobsIn, true, (float) 1, (float) 20, true, (float) 0.60, true,debugIn); // ration W/H=1 +/- 20%
	...
	public Blob[] selectBallBlobs(Blob[]blobsIn) {
		...
		//appelle la fonction avec debug false
		Blob[] myBlobsOut=selectBallBlobs(blobsIn,false); 
...
public void drawConvexityDefects(ConvexityDefect[] cdArrayIn) {
	...
	public Rectangle[] detect() {
		...
		return (detect( Buffer, (float)1.2, 2, opencv_objdetect.CV_HAAR_DO_CANNY_PRUNING, width()/16, height()/16, width(), height(), false  ));
	}
	...
	public Rectangle[] detect(boolean debugIn) {
		...
		return (detect( Buffer, (float)1.2, 2, opencv_objdetect.CV_HAAR_DO_CANNY_PRUNING, width()/16, height()/16, width(), height(), debugIn  ));
	}

	//fonction detect() minimale debug coeff div
	public Rectangle[] detect(int coeffIn, boolean debugIn) {
		
		return (detect( Buffer, (float)1.2, 2, opencv_objdetect.CV_HAAR_DO_CANNY_PRUNING, width()*coeffIn/(16), height()*coeffIn/16, width(), height(), debugIn  ));
	}

	//fonction detect() recevant iplImage
	public Rectangle[] detect(opencv_core.IplImage iplImgIn) {
		
		return (detect( iplImgIn, (float)1.2, 2, opencv_objdetect.CV_HAAR_DO_CANNY_PRUNING, width()/16, height()/16, width(), height(), false  ));
	}

	//fonction detect() recevant iplImage et debug 
	public Rectangle[] detect(opencv_core.IplImage iplImgIn, boolean debugIn) {
		
		return (detect( iplImgIn, (float)1.2, 2, opencv_objdetect.CV_HAAR_DO_CANNY_PRUNING, width()/16, height()/16, width(), height(), debugIn  ));
	}
...
*/
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////
		/////////////////////// FONCTIONS VIDEOS OPENCV ///////////////////////////////////
		///////////////////////////////////////////////////////////////////////////////////////////////
		
		/*
		//----- expérimental -------- 
		
		public void captureInit(int indexIn){
			
			grabber = new OpenCVFrameGrabber(indexIn); // 0 : initialise périph par défaut
	        try {
				grabber.start();
			} catch (Exception e) {
		...
		public PImage capture() throws Exception{
			
			PImage imgOut=null; // déclare PImage à renvoyer
			
	        opencv_core.IplImage grabbedImage = grabber.grab();

	       toPImage(grabbedImage); 
