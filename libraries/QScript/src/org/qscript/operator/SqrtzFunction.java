 ...
 * 
 * 
 * For positive real numbers it has the same output as Math.sqrt(), but for
 * negative numbers it returns the complex root. <br>
 * 
 * @author Peter Lager
 *
 */
public class SqrtzFunction extends Operator {

	public SqrtzFunction(String symbol, int nbrArgs, int priority, int type) {
	...
	public Argument resolve(Script script, Token token, Argument[] args, Object...objects) 
		...
		Argument a0 = args[0];
		if(a0.isNumeric){
			double n = a0.toDouble();
			double root = Math.sqrt(Math.abs(n));
			if(n < 0)
				return new Argument(new Complex(0, root));
			else
				return new Argument(new Double(root));		
		}
		if(a0.isComplex)
