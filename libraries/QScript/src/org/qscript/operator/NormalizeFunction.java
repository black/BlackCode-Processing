...
public class NormalizeFunction extends Operator {

	public NormalizeFunction(String symbol, int nbrArgs, int priority, int type) {
	...
	public Argument resolve(Script script, Token token, Argument[] args, Object... objects) throws EvaluationException {
		...
		Argument a0 = args[0];
		if(a0.isVector)
			return new Argument(((Vector)a0.getValue()).norm());
		if(a0.isComplex)
			return new Argument(((Complex)a0.getValue()).norm());
