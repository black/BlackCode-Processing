...
public class ZViewTextBase {

	// View width and height
	private int vw, vh;

	private PGraphicsJava2D	buffer;
	...
	public ZViewTextBase(PApplet app, int dw, int dh, int wrapwidth, Font font) {
		super();
		this.vw = Math.max(10, dw);
		this.vh = Math.max(10, dh);
		buffer = (PGraphicsJava2D) app.createGraphics(this.vw, this.vh, PApplet.JAVA2D);
		buffer.smooth();
		g2d = buffer.g2;
...
//	}
	...
	/**
	 * Get the Caret for a given pixel position relative to top-left corner of view.
	 * It takes care of any offset for the text (caused by scrolling)
	 * 
	 * @param offX position relative to left edge of view
	 * @param offY position relative to top edge of view
	 * @param caret the caret position matching pixel position
	 * @return
	 */
	public ZCaret getCaretCharPos(float offX, float offY, ZCaret caret){
		if(caret == null)
			caret = new ZCaret();
		float textX = offX - pxlOffsetX;
		float textY = offY - pxlOffsetY;
		LineInfo pli = null;
		caret.valid = false;
		caret.paraNbr = 0;
		for(ZParagraph p : paragraphs){
