...
/**
 * rndV(magnitude) make a random vector of the specified magnitude
 * 
 * @author Peter Lager
 *
 */
public class RandomVectorFunction extends Operator {

	public RandomVectorFunction(String symbol, int nbrArgs, int priority, int type) {
	...
	public Argument resolve(Script script, Token token, Argument[] args, Object...objects) 
		...
		if(a0.isNumeric){
			...
			double rndy = Math.random() - 0.5;
			double rndz = Math.random() - 0.5;

			double factor = a0.toDouble() / Math.sqrt(rndx * rndx + rndy * rndy + rndz * rndz);
			return new Argument(new Vector(rndx * factor, rndy * factor, rndz * factor));
		}
