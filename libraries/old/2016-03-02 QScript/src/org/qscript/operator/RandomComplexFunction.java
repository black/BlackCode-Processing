...
/**
 * rndC(magnitude) make a random complex number of the specified magnitude
 * 
 * @author Peter Lager
 *
 */
public class RandomComplexFunction extends Operator {

	public RandomComplexFunction(String symbol, int nbrArgs, int priority, int type) {
	...
	public Argument resolve(Script script, Token token, Argument[] args, Object...objects) 
		...
		if(a0.isNumeric){
			...
			double mag = a0.toDouble();
			return new Argument(new Complex(mag * Math.cos(ang), mag * Math.sin(ang)));
		}
