/**
 <p>The sketch has two parts the top half displays the 
 selected graph while the bottom half allows the user 
 to setup the parameters for the next plot. When you 
 hit the PLOT button it will attempt to use the values
 supplied to draw the graph, if there are syntax or 
 evaluation errors in the equations entered these will 
 be highlighted. Only when the equations have been 
 successfully evaluated will the graph be updated. </p>
 
 
 <p>This example requires the following additional libraries </p>
 <ul>
 <li>Graphica <a href="https://github.com/jagracar/grafica">GitHub</a></li>
 <li>G4P </li>
 </ul>
 <p>The GUI was created with the GUI Builder tool (G4PTool)</p>
...
public void handleException(GLabel label, GTextField expr, ScriptException e) {
  if (e instanceof SyntaxException || e instanceof EvaluationException) {
