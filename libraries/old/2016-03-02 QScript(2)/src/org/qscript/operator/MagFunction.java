...
public class MagFunction extends Operator {

	public MagFunction(String symbol, int nbrArgs, int priority, int type) {
	...
	public Argument resolve(Script script, Token token, Argument[] args,
		...
		Argument a0 = args[0];
		if(a0.isVector)
			return new Argument(new Double( ((Vector)a0.getValue()).mag()));
		if(a0.isComplex)
			return new Argument(new Double( ((Complex)a0.getValue()).mag()));
